    ...
    , _runningUnitTests     (unitTesting)
{
    ...
    if (_runningUnitTests) {
        ...
        applicationName = QStringLiteral("%1 Daily").arg(QGC_APPLICATION_NAME); // .arg("Q_G_r_o_u_n_d_C_o_n_t_r_o_l");
#else
...
void QGCApplication::setLanguage()
{
    ...
    if (_locale.name() != "en_US") { // если локаль не английская
        QLocale::setDefault(_locale); // установить системную локаль по умолчанию
        if(_qgcTranslatorQtLibs.load("qt_" + _locale.name(), QLibraryInfo::location(QLibraryInfo::TranslationsPath))) {
            ...
            qCWarning(LocalizationLog) << "Qt lib localization for" << _locale.name() << "is not present"; // Локализация Qt lib для "ru_RU" отсутствует
        }
        if(_qgcTranslatorSourceCode.load(_locale, QLatin1String("qgc_source_"), "", ":/i18n")) {
            _app->installTranslator(&_qgcTranslatorSourceCode); // успешная загрузка файла перевода qgc_source_ru_RU.ts
        } else {
        ...
        if(_qgcTranslatorJSON.load(_locale, QLatin1String("qgc_json_"), "", ":/i18n")) {
            _app->installTranslator(&_qgcTranslatorJSON); // успешная загрузка файла перевода qgc_json_ru_RU.ts
        } else {
