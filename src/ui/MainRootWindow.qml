...
/// @brief Собственное окно верхнего уровня QML
/// Все определенные здесь свойства видны на всех страницах QML.
ApplicationWindow {
    id:             mainWindow
    ...
    Component.onCompleted: {
        //-- Полный экран на мобильных или крошечных экранах
        ...
        // Запустите последовательность приглашений первого запуска.
    ...
    //-------------------------------------------------------------------------
    //-- Переменные глобальной области видимости
    ...
    /// Цветовая палитра по умолчанию, используемая во всем пользовательском интерфейсе.
    ...
    //-------------------------------------------------------------------------
    //-- Функции глобальной области видимости

    /// Запретить переключение просмотра
    ...
    /// @return true: Переключение видов в настоящее время не разрешено
    ...
    function showTool(toolTitle, toolSource, toolIcon) { // переключение между режимом полета и режимом составления плана полета
    ...
    function showAnalyzeTool() {
        showTool(qsTr("Analyze Tools"), "AnalyzeView.qml", "/qmlimages/Analyze.svg") // Инструменты анализа
    }

    function showSetupTool() {
        showTool(qsTr("Vehicle Setup"), "SetupView.qml", "/qmlimages/Gears.svg") // Настройка летательного аппарата
    }

    function showSettingsTool() {
        showTool(qsTr("Application Settings"), "AppSettings.qml", "/res/QGCLogoWhite") // Настройки приложения
    }
    ...
    /// Сохраняет положение и размер главного окна
    ...
    property string closeDialogTitle: qsTr("Закрыть %1").arg(QGroundControl.appName)
        ...
        for (var index=0; index<QGroundControl.multiVehicleManager.vehicles.count; index++) {
            if (QGroundControl.multiVehicleManager.vehicles.get(index).parameterManager.pendingWrites) {
                mainWindow.showMessageDialog(closeDialogTitle,
                    qsTr("You have pending parameter updates to a vehicle. If you close you will lose changes. Are you sure you want to close?"), // У вас есть ожидающие обновления параметров летательные аппараты. Если вы закроете, вы потеряете изменения. Вы уверены, что хотите закрыть?
                    StandardButton.Yes | StandardButton.No,
    ...
    function checkForActiveConnections() {
        if (QGroundControl.multiVehicleManager.activeVehicle) {
            mainWindow.showMessageDialog(closeDialogTitle,
                qsTr("There are still active connections to vehicles. Are you sure you want to exit?"), // Есть еще активные подключения к летательным аппаратам. Ты уверен, что хочешь выйти?
                StandardButton.Yes | StandardButton.No,
                function() { finishCloseProcess() })
        } else {
            finishCloseProcess()
    ...
    //-------------------------------------------------------------------------
    /// Фон основного окна во весь экран (Fly View)
    background: Item {
        id:             rootBackground
        anchors.fill:   parent
    }
...
/*//////////    MouseArea {
    ...
    Row {
        ...
        Rectangle {
            ...
            Image {
                anchors.fill: parent
                source: "/qmlimages/minimised.png"
            }
        ...
        Rectangle {
            ...
            Image {
                anchors.fill: parent
                source: "/qmlimages/maximised.png"
            }
    ...
    FlyView { /// все главное окно, без header и footer. Левая вертикальная панель с меню "Полет", авиагоризонт и компас
        id:             flightView
        anchors.fill:   parent
    }

    PlanView { /// окно создания плана
        id:             planView
        anchors.fill:   parent
        visible:        false
    }
    ...
    //-------------------------------------------------------------------------
    //-- Всплывающее окно сообщения о критическом состоянии летательного аппарата
    ...
    Popup {
        ...
        //-- Отклонить сообщение о летательном аппарате
        QGCColoredImage {
